...
EOF
        echo -n "${NC}"
        echo "   ${BL}Created by Ryan Summer${NC}                ${BL}For macOS 10.10-26${NC}           ${BL}v2.1${NC}"
        echo
        echo
...
function uad_resources() {
    while true; do  
        ...
        echo "1) ${GR}Download the latest UAD Software installer${NC} ${BO}${GY}(UAD_11_8_1_75.pkg)${NC}"
        ...
        case $choice in
            1)
                ...
                open https://builds.uaudio.com/apps/uad2/UAD_11_8_1_75.pkg
                ;;
            2) 
...
${GR}${BO}v2.1:${NC}
- Added support for macOS Tahoe 26.0.1
- Added two new menu options:
  · 6) 🔧 ${GR}Check/Fix Permissions${NC}
  · 7) 📚 ${GR}UAD Resources${NC}
- Implemented a permissions check (and optional fix) for cases where a user
  account may not 'own' plugins directories or have write access to move plugins
- the new${NC} 📚 ${GR}UAD Resources${NC} menu option provides quick links to download the
  latest UAD Software${NC} ${BO}'.pkg'${NC} installer ${BO}${GY}(not easily accessible anymore)${NC}, download
  previous installers, check the UAD release notes, or run the uninstaller.
- Now automatically cleans up the UAD_Backup folder (that gets created on the
  desktop) if no plugins were moved - and if nothing exists in the subfolders.
- Removed "UAD MDWEQ5" from the plugin list. This extra PDF is now removed auto-
  matically when both MDWEQ5-3B & MDWEQ5-5B are uncommented in the plugin list.
- Fixed auto-resizing resulting in unintended window dimensions while Terminal's
  font size was set to anything other than '14'.
- Added auto-resize support for iTerm
- Added auto-resize support for 4k resolutions${NC} ${BO}${GY}(now maxes out screen's height)${NC}
- Various other visual enhancements, fixes and optimizations${NC}"
