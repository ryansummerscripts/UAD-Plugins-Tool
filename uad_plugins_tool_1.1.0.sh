...
for plugin in "${plugins[@]}"; do
    ...
    # Move Component file if it exists
    if [ -e "$component_path" ]; then
        mv "$component_path" "$component_dir/" && echo "Moved: $component_path -> $component_dir"
    else
        echo "File not found: $component_path"
    fi
    ...
    for subfolder in "${vst_subfolders[@]}"; do
        ...
        # Move VST file if it exists
        if [ -e "$vst_path" ]; then
            mv "$vst_path" "$vst_dir/" && echo "Moved: $vst_path -> $vst_dir"
        fi

        # Move VST (m) file if it exists
        if [ -e "$vst_path_m" ]; then
            mv "$vst_path_m" "$vst_mono_dir/" && echo "Moved: $vst_path_m -> $vst_mono_dir"
        fi

        # Move VST3 file if it exists
        if [ -e "$vst3_path" ]; then
            mv "$vst3_path" "$vst3_dir/" && echo "Moved: $vst3_path -> $vst3_dir"
        else
            echo "File not found: $vst3_path"
        fi
    done

    # Move AAX file if it exists
    if [ -e "$aax_path" ]; then
        mv "$aax_path" "$aax_dir/" && echo "Moved: $aax_path -> $aax_dir"
    else
        echo "File not found: $aax_path"
    fi

    # Move Documentation file using find to match partial names
    find "$docs_base" -type f -iname "*${plugin#UAD }*.pdf" -exec mv {} "$docs_dir/" \; -exec echo "Moved: {} -> $docs_dir" \; || echo "Documentation not found for: $plugin"
    done

    # Move the UAD Plug-Ins Manual.pdf file if it exists
    uad_manual="$docs_base/UAD Plug-Ins Manual.pdf"
    if [ -e "$uad_manual" ]; then
        mv "$uad_manual" "$docs_dir/" && echo "Moved: $uad_manual -> $docs_dir"
    else
        echo "File not found: $uad_manual"
    fi
