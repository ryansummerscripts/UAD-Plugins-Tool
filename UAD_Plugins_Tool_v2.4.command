...
#    "UAD elysia karacter master"
#    "UAD elysia karacter mix"
#    "UAD elysia mpressor"
...
EOF
    echo -n "${NC}"
    echo "   ${BL}Created by Ryan Summer${NC}                ${BL}For macOS 10.10-26${NC}           ${BL}v2.4${NC}"
}
# Navigation prompt
    ...
    while true; do
        ...
        while true; do
            ...
            for uad_plugin in "${plugins[@]}"; do
                ...
                # === Catch VST/VST3 files placed directly in the Universal Audio
                #     root, outside any of the known category subfolders ===
                vst_root_path="$vst_base/${uad_plugin}.vst"
                vst_root_path_m="$vst_base/${uad_plugin}(m).vst"
                vst3_root_path="$vst3_base/${uad_plugin}.vst3"

                # VST (root)
                if [ -e "$vst_root_path" ]; then
                    if mv "$vst_root_path" "$vst_dir/" 2>/dev/null; then
                        echo "🗑️  ${GR}VST moved for:${NC} ${GY}${BO}..........${NC} ${GR}$uad_plugin${NC}"
                        ((vst_count++))
                    elif sudo mv "$vst_root_path" "$vst_dir/"; then
                        echo "🗑️  ${GR}VST moved for:${NC} ${GY}${BO}..........${NC} ${GR}$uad_plugin${NC}"
                        ((vst_count++))
                    fi
                fi
                ...
                # VST (m) (root)
                if [ -e "$vst_root_path_m" ]; then
                    if mv "$vst_root_path_m" "$vst_mono_dir/" 2>/dev/null; then
                        echo "🗑️  ${GR}VST (m) moved for:${NC} ${GY}${BO}......${NC} ${GR}$uad_plugin${NC}"
                        ((vst_mono_count++))
                    elif sudo mv "$vst_root_path_m" "$vst_mono_dir/"; then
                        echo "🗑️  ${GR}VST (m) moved for:${NC} ${GY}${BO}......${NC} ${GR}$uad_plugin${NC}"
                        ((vst_mono_count++))
                    fi
                fi
                ...
                # VST3 (root)
                if [ -e "$vst3_root_path" ]; then
                    if mv "$vst3_root_path" "$vst3_dir/" 2>/dev/null; then
                        echo "🗑️  ${GR}VST3 moved for:${NC} ${GY}${BO}.........${NC} ${GR}$uad_plugin${NC}"
                        ((vst3_count++))
                    elif sudo mv "$vst3_root_path" "$vst3_dir/"; then
                        echo "🗑️  ${GR}VST3 moved for:${NC} ${GY}${BO}.........${NC} ${GR}$uad_plugin${NC}"
                        ((vst3_count++))
                    fi
                fi
    ...
    while true; do    
        ...
        while true; do
            ...
            for uad_plugin in "${plugins[@]}"; do
                ...
                # === Check VST/VST3 files placed directly in the Universal Audio
                #     root, outside any of the known category subfolders ===
                vst_root_path="$vst_base/${uad_plugin}.vst"
                vst_root_path_m="$vst_base/${uad_plugin}(m).vst"
                vst3_root_path="$vst3_base/${uad_plugin}.vst3"
...
function resources() {
    while true; do  
        ...
        show_navigation_prompt_centered  # already padded
        echo "🔗 ${BO}Links:${NC}"
        echo " 1) ${GR}Download the latest UAD Software installer${NC} ${BO}${GY}(UAD_11_9_0_515.pkg)${NC}"
        ...
        case $choice in
            1)
                ...
                open https://builds.uaudio.com/apps/uad2/UAD_11_9_0_515.pkg
                ;;
            2) 
